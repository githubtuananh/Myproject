$(document).ready(function(){
    ...
    //Xử lý loading icon trang
    $(window).ready(function(){
        $('.loader-computer').delay(700).fadeOut('slow');
        $('.loader-mobile').delay(700).fadeOut('slow');
    })

    //Xử lý mọi thay đổi khi thay đổi kích thước trang
    $(window).resize(function(){
        var wWindow = $(window).width()
        if(wWindow > 45*16){
            $('.loader-computer').css('display','none')
            $(document).find('.container.mobile .page-register').remove()
            $(document).find('.container.mobile .page-login').remove()
        }
        else{
    ...
    // Xử lý giao diện ajax login và register giao diện
    ...
    //Xử lý giao diện login register 
    $('.slide-user .login.mobile').click(function(){
        history.pushState(null, null, "/demoGooglePlay/store/login")
        $.get('/demoGooglePlay/ajax/login',function(data){
    ...
    $('.slide-user .register.mobile').click(function(){
        history.pushState(null, null, "/demoGooglePlay/store/register")
        $.get('/demoGooglePlay/ajax/register',function(data){
            // $('.item-container').css('display','none')
            $('.cover-main').fadeOut('fast');
    ...
    //Xử lý load dữ liệu của trang mobile
